...
3. Clicked on Install for the device:
   - Android: Installed from the Google Play Store.
   - iOS: Installed from the Apple App Store.
4. Opened the Expo Go app on the device.
5. Created a new account or logged in if already having one.

### Testing the Setup

- [ ] Installed Expo Go successfully.
- [ ] Logged in or created account.
- [ ] Ready to proceed with React Native development using Expo.

## Challenges Faced

- No challenges encountered during the setup process.
- Ensure the device and computer are on the same network for testing apps via Expo Go.

## Notes

This setup allows for efficient mobile app development without the need for expensive emulators or multiple physical devices.